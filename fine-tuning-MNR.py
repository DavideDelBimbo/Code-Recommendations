...
class CodeExampleTrainer:
    ...
    def __batch_size(self, batch) -> float:
        ...
        """        
        noised_features: torch.Tensor = [b for b in batch[0][0].values()]
        original_features: torch.Tensor = [b for b in batch[0][1].values()]
    ...
    def load_model(self) -> None:
        ...
        word_embedding_model = models.Transformer(self.model_name_or_path, max_seq_length=None)

        # Apply mean pooling to get one fixed sized vector.
    ...
    def load_dataset(self, file_path='./datasets/query-code_examples.json') -> None:
        """
        Load dataset of questions and code examples from a JSON file.
        
        Parameters:
            - file_path (str, optional): path to code examples dataset JSON file (default './datasets/query-code_examples.json').
        """        
        start = time.time()
        print("Loading dataset...")

        # Load JSON dataset as DataFrame object.
        self.dataset = pd.read_json(file_path, encoding='utf-8')
    ...
    def preprocess_data(self) -> None:
        ...
        """
        # Preprocess pairs (query, code example) from dataset.
        for i, example in tqdm(self.dataset.iterrows(), total=len(self.dataset), desc='Preprocessing', smoothing=0.05, dynamic_ncols=True):
            # Remove tag from query.
            query = re.sub(r"[\t\n]", "", example['Query'])
            query = re.sub(r"\s+", " ", example['Query'])

            # Remove tag from code examples.
            code = re.sub(r"[\t\n]", "", example['CodeExample'])
            code = re.sub(r"\s+", " ", example['CodeExample'])

            # Replace cleaned data.
            self.dataset.loc[i, 'Query'] = query
            self.dataset.loc[i, 'CodeExample'] = code        
    ...
    def fine_tune_model(self, batch_size: int = 4, epochs: int = 5, evaluation_steps: int = 1000, output_path: str = './model/') -> None:
        ...
        train_split, dev_split = model_selection.train_test_split(self.dataset, test_size=0.2, random_state=self.seed, shuffle=True)

        # Create training dataset from pair (query, code example).
        self.train_set = [InputExample(texts=(example['Query'], example['CodeExample'])) for _, example in train_split.iterrows()]
        # Create DataLoader from training dataset.
        self.train_dataloader = datasets.NoDuplicatesDataLoader(self.train_set, batch_size=batch_size)
        # Transforms a batch from a DataLoader to a batch of tensors for the model.
        self.train_dataloader.collate_fn = self.model.smart_batching_collate


        # Create development dataset from pair (query, code example).
        self.dev_set = [InputExample(texts=(example['Query'], example['CodeExample'])) for _, example in dev_split.iterrows()]
        # Create DataLoader from development dataset.
        self.dev_dataloader = datasets.NoDuplicatesDataLoader(self.dev_set, batch_size=batch_size)
        # Transforms a batch from a DataLoader to a batch of tensors for the model.
        self.dev_dataloader.collate_fn = self.model.smart_batching_collate
        

        # Define loss function.
        self.train_loss = LossTrain(model=self.model, evaluation_steps=evaluation_steps, epoch_steps=len(self.train_dataloader), log_dir=self.log_dir)
        # Load loss function on GPU if available.
        self.train_loss.to(self.device)


        # Define evaluator.
        self.evaluator = LossEvaluator(dataloader=self.dev_dataloader, loss=self.train_loss, epoch_steps=len(self.train_dataloader), show_progress_bar=True, write_csv=True, output_name='dev', log_dir=self.log_dir)
...
class LossTrain(losses.MultipleNegativesRankingLoss):
...
class LossEvaluator(evaluation.SentenceEvaluator):
    def __init__(self, dataloader: DataLoader, loss: LossTrain, epoch_steps: int, show_progress_bar: bool = False, write_csv: bool = True, output_name: str = '', log_dir: str = None) -> None:
        ...
        Parameters:
            ...
            - loss (LossTrain, optional): loss module object.
            - epoch_steps (int): number of steps between each epoch.
...
def main():
    code_trainer = CodeExampleTrainer(model_name_or_path='./model (TSDE)/', log_dir='./logs/')
    code_trainer.load_model()
    code_trainer.load_dataset()
    code_trainer.preprocess_data()
    code_trainer.fine_tune_model(batch_size=6, epochs=100, evaluation_steps=1000, output_path='./model (TSDE + MNR)/')
